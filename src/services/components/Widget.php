    ...
    ScaffoldCommandInterface
{
    ...
    {
        $widget_paths = array();
        array_walk($this->component_strings, function ($component_string) use(&$widget_paths) {
            $widget_paths[] = "{$this->addon_string->constants()->path()} "
                              . ". 'widget/EEW_{$component_string->package()}.widget.php'";
        });
        return array("'widget_paths' => " . Template::formattedArrayString($widget_paths, 5));
    }
     ...
     *
     * When called individually this command only generates the files related to widgets but does not generate
     * items included in main addon code, nor does it generate the main addon scaffold.  It's purpose is for when
     * you want to quickly add the scaffold for additional widgets after the fact.
     *
     * ## Options
     *
     * <addon_slug>
     * : The slug of the addon the widgets are being added to.
     *
     * [--widget=<widget_slugs>]
     * : Comma-delimited list of slugs for each set of widget elements you want created.
     *
     *
     * [--addon_author=<name>]
     ...
     * ## Examples
     *
     *      # Generate Widgets for the slugs event_authors and event_people for my-awesome-addon.
     *      $ wp ee scaffold widget my-awesome-addon --widget=event_authors,event_people
     *      Success: Files created.
     *      /path/to/wp-plugins/eea-my-awesome-addon/widget/EEW_Event_Authors.widget.php
     *      /path/to/wp-plugins/eea-my-awesome-addon/widget/EEW_Event_People.widget.php
     *
     */
    ...
    {
        return 'Generate starter files and code for widgets that are part of an Event Espresso Addon';
    }
        ...
        if ($skip_global) {
            return array(
                array(
                    'type'        => 'assoc',
                    'name'        => 'widget',
                    'description' => 'A comma-delimited list of slugs for widgets you\'d like created.',
                    'optional'    => true,
                ),
            );
        } else {
